...
public interface Connection {
	...
	/**
	 * Creates a new request, using this Connection as the session-state and to
	 * initialize the connection settings (which may then be independently on the
	 * returned Connection.Request object).
	 * 
	 * @return a new Connection object, with a shared Cookie Store and initialized
	 *         settings from this Connection and Request
	 * @since 1.14.1
	 */
	...
	/**
	 * Set the proxy to use for this request. Set to <code>null</code> to disable a
	 ...
	 * <p>
	 * The default timeout is <b>30 seconds</b> (30,000 millis). A timeout of zero
	 ...
	 * 
	 * @param millis number of milliseconds (thousandths of a second) before timing
	 ...
	 * body, before the connection is closed, and the input truncated (i.e. the body
	 * content will be trimmed). <b>The default maximum is 2MB</b>. A max size of
	 * <code>0</code> is treated as an infinite amount (bounded only by your
	 * patience and the memory available on your machine).
	 *
	...
	/**
	 * Configures the connection to (not) follow server redirects. By default this
	 * is <b>true</b>.
	 * 
	 ...
	 * (4xx - 5xx, e.g. 404 or 500). By default this is <b>false</b>; an IOException
	 * is thrown if an error is encountered. If set to <b>true</b>, the response is
	...
	/**
	 * Ignore the document's Content-Type when parsing the response. By default this
	 * is <b>false</b>, an unrecognised content-type will cause an IOException to be
	 * thrown. (This is to prevent producing garbage by attempting to parse a JPEG
	 * binary image, for example.) Set to true to force a parse attempt regardless
	 * of content type.
	 * 
	 ...
	 * query string for GETs, and in the request body for POSTs. A request may have
	...
	/**
	 * Add an input stream as a request data parameter. For GETs, has no effect, but
	 * for POSTS this will upload the input stream.
	 * 
	 ...
	 *                    from a {@link java.io.FileInputStream}. You must close the
	 ...
	 * </p>
	 * <p>
	 * For GET requests, data parameters will be sent on the request query string.
	 * For POST (and other methods that contain a body), they will be sent as body
	 * form parameters, unless the body is explicitly set by
	 * {@link #requestBody(String)}, in which case they will be query string
	 * parameters.
	 * </p>
	 * 
	 ...
	 * request body, not a set for URL encoded form key/value pairs. E.g.:
	 * <code><pre>Jsoup.connect(url)
	 * .requestBody(json)
	 * .header("Content-Type", "application/json")
	 * .post();</pre></code> If any data key/vals are supplied, they will be sent as
	 * URL query params.
	 * 
	...
	/**
	 * Provide a custom or pre-filled CookieStore to be used on requests made by
	 * this Connection.
	 * 
	 ...
	 * If not set, defaults to the HTML parser, unless the response content-type is
	 * XML, in which case the XML parser is used.
	 * 
	 ...
	 * @throws java.net.MalformedURLException  if the request URL is not a HTTP or
	 *                                         HTTPS URL, or is otherwise malformed
	 * @throws HttpStatusException             if the response is not OK and HTTP
	 *                                         response errors are not ignored
	 * @throws UnsupportedMimeTypeException    if the response mime type is not
	 *                                         supported and those errors are not
	 *                                         ignored
	 ...
	 * 
	 * @return response
	 * @throws IllegalArgumentException if called before the response has been
	 *                                  executed.
	 */
	Response response();
	...
	interface Base<T extends Base<T>> {
		/**
		 * Get the URL of this Request or Response. For redirected responses, this will
		 * be the final destination URL.
		 * 
		 * @return URL
		 * @throws IllegalArgumentException if called on a Request that was created
		 *                                  without a URL.
		 */
		URL url();
		...
		/**
		 * Get the value of a header. If there is more than one header value with the
		 * same name, the headers are returned comma seperated, per <a href=
		...
		/**
		 * Set a header. This method will overwrite any existing header with the same
		 * case insensitive name. (If there is more than one value for this header, this
		...
		/**
		 * Add a header. The header will be added regardless of whether a header with
		...
		/**
		 * Remove headers by name. If there is more than one header with this name, they
		 * will all be removed.
		 * 
		...
		/**
		 * Retrieve all of the request/response header names and corresponding values as
		 * a map. For headers with multiple values, only the first header is returned.
		 * <p>
		 * Note that this is a view of the headers only, and changes made to this map
		 * will not be reflected in the request/response object.
		 * </p>
		 * 
		 * @return headers
		 * @see #multiHeaders()
		...
		/**
		 * Retreive all of the headers, keyed by the header name, and with a list of
		 * values per header.
		 * 
		 ...
		 * response is added to the response object's cookie key=value map. The cookie's
		 * path, domain, and expiry date are ignored.
		 * </p>
		 * 
	...
	interface Request extends Base<Request> {
		...
		/**
		 * Configures the request to (not) follow server redirects. By default this is
		 * <b>true</b>.
		 * 
		 * @param followRedirects true if server redirects should be followed.
		 * @return this Request, for chaining
		 ...
		 * 
		 * @return true if errors will be ignored; false (default) if HTTP errors will
		 ...
		 * 
		 * @return true if invalid content-types will be ignored; false (default) if
		 *         they will cause an IOException to be thrown.
		 */
		 ...
		 * URL query params.
		 * 
		 * @param body to use as the request body. Set to null to clear a previously set
		 *             body.
		 * @return this Request, for chaining
		 */
	...
	interface Response extends Base<Response> {
		...
		/**
		 * Get the character set name of the response, derived from the content-type
		 * header.
		 * 
		...
		/**
		 * Set / override the response character set. When the document body is parsed
		 * it will be with this charset.
		 * 
		...
		/**
		 * Read and parse the body of the response as a Document. If you intend to parse
		 * the same response multiple times, you should {@link #bufferUp()} first.
		 * 
		...
		/**
		 * Read the body of the response into a local buffer, so that {@link #parse()}
		 * may be called repeatedly on the same connection response (otherwise, once the
		 * response is read, its InputStream will have been drained and may not be
		 * re-read). Calling {@link #body() } or {@link #bodyAsBytes()} has the same
		 * effect.
		 * 
		 * @return this response, for chaining
		...
		/**
		 * Get the body of the response as a (buffered) InputStream. You should close
		 * the input stream when you're done with it. Other body methods (like bufferUp,
		 * body, parse, etc) will not work in conjunction with this method.
		 * <p>
		 * This method is useful for writing large responses to disk, without buffering
		 * them completely into memory first.
		 * </p>
		 * 
	...
	interface KeyVal {
		 ...
		 * uploading files. Only useful if {@link #inputStream(InputStream)} is set.
		 * <p>
