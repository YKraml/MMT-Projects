    ...
    public double simpsonsMethod(int N, double h, double a) {
        ...
        for (int j = 0; j < data.size(); j++) {
            if (j == 0 || j == data.size() - 1) {
                integralEvaluation += data.get(j);
                System.out.println("Multiply f(x" + j + ") by 1");
            } else if (j % 2 == 1) {
                integralEvaluation += (double) 4 * data.get(j);
                System.out.println("Multiply f(x" + j + ") by 4");
            } else {
                integralEvaluation += (double) 2 * data.get(j);
                System.out.println("Multiply f(x" + j + ") by 2");
            }
        }
